...
async function translateFile(file, targetLang) {
   ...
   const untranslated = _.merge({}, rawSource, excludedSource);
   const translated = {};
...
This JSON contains interface text for a ticket app.
...
- **If a key contains or ends with "id"**:
  ...
  - If "id" is a technical suffix (e.g., "product_id", "clientId"), **only translate the prefix** (e.g., "Product", "Client").
...
- Use a **formal and professional tone** suitable for e-commerce software interfaces.
- Use **title casing** for labels and messages when appropriate.
- Make translations as short as possible.
...
function runVueI18nExtract() {
   return new Promise((resolve, reject) => {
      ...
      const extract = spawn(
         'npx vue-i18n-extract report --vueFiles "./src/**/*.?(ts|js|vue)" --languageFiles "./src/plugins/i18n/locales/**/!(*.exclude).?(json)" --add --remove',
         {
            shell: true,
            stdio: 'inherit'
         }
...
async function run() {
   if ((await isOllamaAvailable()) === false) {
      console.warn("⚠️ Ollama isn't available. Skipping translation.");
      return;
   }
